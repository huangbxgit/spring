package hbx.spring.formework.webmvc.servlet;
...
import hbx.spring.formework.aop.MyProxyUtils;
import hbx.spring.formework.context.MyApplicationContext;
import hbx.spring.formework.webmvc.MyHandlerAdapter;
import hbx.spring.formework.webmvc.MyHandlerMapping;
import hbx.spring.formework.webmvc.MyModelAndView;
import hbx.spring.formework.webmvc.MyViewResolver;
...
//    private Map<String,MyHandlerMapping> handlerMapping = new HashMap<String,MyHandlerMapping>();
    ...
    protected void initStrategies(MyApplicationContext context) {
        ...
        /** 我们自己会实现 */
        //MyHandlerMapping 用来保存Controller中配置的RequestMapping和Method的一个对应关系
    ...
    private void initHandlerMappings(MyApplicationContext context) {
        ...
        for (String beanName : beanNames) {
            Object proxyObj = context.getBean(beanName);
            Object controller = MyProxyUtils.getTargetObject(proxyObj);
    ...
    private void initHandlerAdapters(MyApplicationContext context) {
        ...
        for (MyHandlerMapping handlerMapping : this.handlerMappings){
            ...
            this.handlerAdapters.put(handlerMapping,new MyHandlerAdapter(paramMapping));
        }
    ...
    private void initViewResolvers(MyApplicationContext context) {
        ...
        for (File template : templateRootDir.listFiles()) {
            this.viewResolvers.add(new MyViewResolver(template.getName(),template));
        }
...
//        MyHandlerMapping handler = handlerMapping.get(url);

//        try {
//            MyModelAndView mv = (MyModelAndView)handler.getMethod().invoke(handler.getController());
